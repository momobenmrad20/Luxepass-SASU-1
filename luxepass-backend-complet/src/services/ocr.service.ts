...
// Permis de conduire) via la vision de Claude, à l'étape 2 du check-in
...
// vers /v1/messages, pas de dépendance @anthropic-ai/sdk ajoutée —
// le format JSON envoyé est de toute façon identique à ce que ferait
// le SDK, et ça reste cohérent avec le reste du backend.
//
// ─────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────
// Appel bas niveau à /v1/messages — même pattern que callAnthropic()
// dans ai.service.ts, avec un content block image/document au lieu
// d'un simple message texte.
// ─────────────────────────────────────────────────────────────
async function callAnthropicVision(fileBuffer: Buffer, mimeType: string): Promise<string> {
  if (!config.ai.anthropicApiKey) {
    throw new OcrServiceError("Clé ANTHROPIC_API_KEY absente — OCR pièce d'identité désactivé");
  }

  const base64Data = fileBuffer.toString("base64");

  // PDF : content block "document" natif — l'API Anthropic gère elle-même
  // le rendu des pages du PDF, pas besoin de le rasteriser ni d'en
  // extraire la première page côté serveur (pas de dépendance PDF de plus
  // à maintenir : pdf-lib, pdf-poppler, etc.).
  ...
  const documentBlock = isPdf
    ...
    : {
        ...
        source: {
          ...
          // media_type resserré à ce que fileFilter (upload.ts) autorise déjà.
  ...
  try {
        ...
        messages: [
          {
            ...
            content: [
              documentBlock,
              { type: "text", text: "Analyse ce document d'identité et réponds uniquement avec le JSON demandé." },
            ],
          },
        ],
      }),
    ...
    throw new OcrServiceError("Impossible de contacter l'API Anthropic (réseau)");
  }

  if (!res.ok) {
    const body = await res.text().catch(() => "");
    throw new OcrServiceError(`Anthropic API erreur ${res.status}: ${body.slice(0, 300)}`);
  }

  const data = (await res.json()) as { content?: Array<{ type: string; text?: string }> };
  const text = data.content?.find((b) => b.type === "text")?.text;
  if (!text) {
    throw new OcrServiceError("Réponse Anthropic vide ou dans un format inattendu");
  }
  return text;
}
...
export async function processIdDocument(
  ...
  const raw = await callAnthropicVision(fileBuffer, mimeType);
  const parsed = safeParseOcrJson(raw);
  if (!parsed) {
    // Réponse "réussie" côté API mais JSON exploitable → dégradation
